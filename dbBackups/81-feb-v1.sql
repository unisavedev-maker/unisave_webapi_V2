...
BEGIN
    ...
    ELSE
        ...
        IF v_user_id IS NULL THEN
            ...
            SET v_user_id = LAST_INSERT_ID();
          call sp_users_profile_save ('SAVE',0,v_user_id,p_name,'','','','','',100,0,'no-image.png');
        END IF;
...
BEGIN
    ...
    ELSE
           ...
           update share_requests set status=3,requested_percent=0,responded_at =now() where request_id = p_request_id;
            WHEN 'GUEST_CONTRIBUTION_SUMMARY' THEN
           select * from vw_participentdetail where requester_id =v_target_u_id and share_typeid=2;
...
/*!50003 SET character_set_results = @saved_cs_results */ ;
/*!50003 SET collation_connection  = @saved_col_connection */ ;
/*!50003 DROP PROCEDURE IF EXISTS `sp_update_image_by_type` */;
/*!50003 SET @saved_cs_client      = @@character_set_client */ ;
/*!50003 SET @saved_cs_results     = @@character_set_results */ ;
/*!50003 SET @saved_col_connection = @@collation_connection */ ;
/*!50003 SET character_set_client  = utf8mb4 */ ;
...
/*!50003 SET sql_mode              = 'ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`devom`@`%` PROCEDURE `sp_update_image_by_type`(
    IN p_image_type INT,
    IN record_id INT,
...
BEGIN
    ...
    CASE p_image_type
        ...
        WHEN 1 THEN
            UPDATE users_profile
            SET user_image = p_image
            WHERE profileidpk = record_id;

            SET v_rows = ROW_COUNT();
   WHEN 7 THEN
            UPDATE tbl_particiepents_master
            SET image_name = p_image
            WHERE p_id = record_id;

            SET v_rows = ROW_COUNT();
        ELSE
            SIGNAL SQLSTATE '45000'
            SET MESSAGE_TEXT = 'Invalid image_type';
...
/*!50003 SET character_set_results = @saved_cs_results */ ;
/*!50003 SET collation_connection  = @saved_col_connection */ ;
/*!50003 DROP PROCEDURE IF EXISTS `sp_users_profile_save` */;
/*!50003 SET @saved_cs_client      = @@character_set_client */ ;
/*!50003 SET @saved_cs_results     = @@character_set_results */ ;
/*!50003 SET @saved_col_connection = @@collation_connection */ ;
/*!50003 SET character_set_client  = utf8mb4 */ ;
...
/*!50003 SET sql_mode              = 'ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`devom`@`%` PROCEDURE `sp_users_profile_save`(
    IN p_action VARCHAR(10),   -- SAVE / DELETE / GET / GETALL
    IN p_profileidpk BIGINT,
    IN p_usersidpkfk BIGINT,
    ...
    IN p_province VARCHAR(100),
    IN p_postalcode VARCHAR(20),
    IN p_max_allocation DECIMAL(10,2),
    IN p_allocation DECIMAL(10,2),
    IN p_user_image VARCHAR(255)
)
BEGIN

    DECLARE v_existing_id BIGINT DEFAULT 0;

    -- Check existing profile by user id
    SELECT profileidpk INTO v_existing_id
    FROM users_profile
    WHERE usersidpkfk = p_usersidpkfk
    AND isDeleted = 0
    LIMIT 1;
    ...
    CASE
        ...
        WHEN p_action = 'SAVE' THEN
            ...
            IF v_existing_id IS NOT NULL AND v_existing_id > 0 THEN
                ...
                WHERE profileidpk = v_existing_id;

                SELECT 'UPDATED' AS status, v_existing_id AS profile_id;
            ...
            ELSE
             ...
             /*   SELECT 'INSERTED' AS status, LAST_INSERT_ID() AS profile_id;*/
        ...
        -- ================= GET ALL =================
        WHEN p_action = 'GETALL' THEN

            SELECT *
            FROM users_profile
            WHERE isDeleted = 0 and usersidpkfk = p_usersidpkfk;
...
-- Dump completed on 2026-02-18 13:41:45
